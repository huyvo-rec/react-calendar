...
function CalendarToolbar(props: any) {
  ...
  return useMemo(() => {
    ...
    const firstDayOfWeekMonth = format(selectedWeek[0], "MMMM", {
      locale: locale,
    });
    const lastDayOfWeekMonth = format(selectedWeek[6], "MMMM", {
      locale: locale,
    });
    ...
    return (
      ...
      >
        <Toolbar>
          ...
          <Tooltip
            title={`${format(new Date(), "EEEE, d MMMM", { locale: locale })}`}
